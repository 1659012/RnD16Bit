...
void setup() {
  ...
  Serial.println("Choose test command:");
  Serial.println("a. Print the quadcopter angle");
  Serial.println("r. Reading receiver signals.");
  Serial.println("1. Test motor 1 (right front CCW.)");
  Serial.println("2. Test motor 2 (right rear CW.)");
  Serial.println("3. Test motor 3 (left rear CCW.)");
  Serial.println("4. Test motor 4 (left front CW.)");
  Serial.println("5. Test all motors together");
...
void loop() {
  ...
  if (data == '1' || data == '2' || data == '3' || data == '4' || data == '5') {        //If motor 1, 2, 3 or 4 is selected by the user.
    Serial.println("Begin test motor");
    ...
    if (new_function_request == false) {                                                //When the throttle was in the lowest position do this.
      ...
      if (eeprom_data[31] == 1) {                                                       //The MPU-6050 is installed
        Serial.println("Start communication with the gyro");
        Wire.beginTransmission(gyro_address);                                           //Start communication with the gyro.
        Wire.write(0x3B);                                                               //Start reading @ register 43h and auto increment with every read.
        Wire.endTransmission(stop);                                                         //End the transmission.
        Serial.println("Start reading @ register 43h and auto increment with every read");

        Wire.requestFrom(gyro_address, 6);                                              //Request 6 bytes from the gyro.
        Serial.println("Request 6 bytes from the gyro");
        while (Wire.available() < 6);                                                   //Wait until the 6 bytes are received.
        Serial.println("Wait until the 6 bytes are received.");
...
void set_gyro_registers() {
  ...
  if (eeprom_data[31] == 1) {
    ...
    Wire.endTransmission(stop);                                      //End the transmission with the gyro
    Serial.println("write to the GYRO_CONFIG register");

    Wire.beginTransmission(gyro_address);                        //Start communication with the address found during search.
    Wire.write(0x1C);                                            //We want to write to the ACCEL_CONFIG register (1A hex)
    Wire.write(0x10);                                            //Set the register bits as 00010000 (+/- 8g full scale range)
    Wire.endTransmission(stop);                                      //End the transmission with the gyro
    Serial.println("write to the ACCEL_CONFIG register");
  ...
  Serial.println("End setup the gyro-MPU-6050");
}
